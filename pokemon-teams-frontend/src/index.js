...
function getAllTrainersAndPokemons() {
    ...
    return fetch(TRAINERS_URL, requestConfigs)
...
function getPokemonFromApi(pokemon, cardDiv) {
    ...
    return fetch(`${POKEMONS_URL}/${pokemon.id}`, requestConfigs)
    .then(function (response) {
        return response.json();
    }).then(function (json) {
...
function addPokemonToList(pokemon) {
    ...
    liElement.textContent = `${pokemon.attributes.nickname} (${pokemon.attributes.species})`
    liElement.appendChild(releasePkmBtn)
    return liElement
